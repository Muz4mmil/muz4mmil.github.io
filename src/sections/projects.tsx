						...
						>
							<div className="lg:w-1/2 w-full text-black space-y-5">
								...
								<div className="flex gap-4 pt-4">
									<a href={project.code} target='_blank' className='flex items-center border border-white hover:border-neutral-300 rounded-full px-3 py-1 group/buttons'>Code <ArrowUpRight size={16} className='ml-1 group-hover/buttons:rotate-45 duration-300' /></a>
									<a href={project.link} target='_blank' className='flex items-center border border-white hover:border-neutral-300 rounded-full px-3 py-1 group/buttons'>Preview <ArrowUpRight size={16} className='ml-1 group-hover/buttons:rotate-45 duration-300' /></a>
								</div>
							</div>
							...
							<div className="relative lg:w-1/2 w-full h-72 sm:h-96 overflow-hidden bg-gradient-to-br from-neutral-50 to-neutral-100 rounded-md">
								<div className="h-full w-full">
									...
									<img
										...
										alt={project.name}
										className={`h-full w-full object-contain p-10 grayscale hover:grayscale-0 transition-all duration-700 ease-in-out ${!index && 'object-cover p-[0px]'}`}
									/>
								</div>
							</div>
						</motion.div>
